...
pub enum Insn {
    ...
    /// Get local varaible from non-immediate scope
    GetLocal { level: NonZeroU32, ep_offset: u32 },
    /// Set a local varaible in a non-immediate scope
...
impl<'a> std::fmt::Display for InsnPrinter<'a> {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        match &self.inner {
            ...
            Insn::GetLocal { level, ep_offset } => write!(f, "GetLocal l{level}, {ep_offset}"),
            Insn::SetLocal { val, level, ep_offset } => write!(f, "SetLocal l{level}, {ep_offset}, {val}"),
...
pub enum ParseError {
    ...
    MalformIseq(u32), // insn_idx into iseq_encoded
}
    ...
    while let Some((incoming_state, block, mut insn_idx)) = queue.pop_front() {
        ...
        while insn_idx < iseq_size {
            ...
            match opcode {
                ...
                YARVINSN_getlocal => {
                    let ep_offset = get_arg(pc, 0).as_u32();
                    let level = NonZeroU32::try_from(get_arg(pc, 1).as_u32()).map_err(|_| ParseError::MalformIseq(insn_idx))?;

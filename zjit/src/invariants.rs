...
pub extern "C" fn rb_zjit_cme_invalidate(cme: *const rb_callable_method_entry_t) {
    ...
    with_vm_lock(src_loc!(), || {
        let invariants = ZJITState::get_invariants();
        if let Some(jumps) = invariants.cme_patch_points.get(&cme) {
            let cb = ZJITState::get_code_block();
        ...
        // Clean up the tracking entry since the CME is no longer valid
        invariants.cme_patch_points.remove(&cme);
    });
}

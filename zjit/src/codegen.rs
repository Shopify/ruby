        ...
        for &block_id in reverse_post_order.iter() {
            ...
            for (idx, &insn_id) in block.params().enumerate() {
                ...
                match crate::hir::ResolvedInsnId(insn_id).insn(function) {
                    Insn::Param => {
                        jit.opnds[insn_id] = Some(runtime::gen_param(&mut asm, idx));
                    },
            ...
            if function.is_entry_block(block_id) {
                for &insn_id in block.insns() {
                    ...
                    if let &Insn::LoadArg { idx, .. } = crate::hir::ResolvedInsnId(insn_id).insn(function) {
                        jit.opnds[insn_id] = Some(runtime::gen_param(&mut asm, idx as usize));
                    }
                }
            }
...
fn gen_insn(cb: &mut CodeBlock, jit: &mut JITState, asm: &mut Assembler, function: &Function, insn_id: InsnId, insn: &Insn) -> Result<(), InsnId> {
    ...
    let out_opnd = match insn {
        ...
        &Insn::Const { val: Const::Value(val) } => runtime::gen_const_value(val),
        &Insn::Const { val: Const::CPtr(val) } => runtime::gen_const_cptr(val),
        &Insn::Const { val: Const::CInt64(val) } => runtime::gen_const_long(val),
        &Insn::Const { val: Const::CUInt16(val) } => runtime::gen_const_uint16(val),
        &Insn::Const { val: Const::CUInt32(val) } => runtime::gen_const_uint32(val),
        &Insn::Const { val: Const::CUInt64(val) } => Opnd::UImm(val),
        &Insn::Const { val: Const::CAttrIndex(val) } => runtime::gen_const_attr_index_t(val),
        &Insn::Const { val: Const::CShape(val) } => {
            assert_eq!(SHAPE_ID_NUM_BITS, 32);
            runtime::gen_const_uint32(val.0)
        }
        ...
        Insn::NewHash { elements, state } => {
            ...
            objects::gen_new_hash(jit, asm, function, opnds!(elements), sym_keys, &function.frame_state(*state))
        }
        Insn::NewRange { low, high, flag, state } => objects::gen_new_range(jit, asm, function, opnd!(low), opnd!(high), *flag, &function.frame_state(*state)),
        Insn::NewRangeFixnum { low, high, flag, state } => objects::gen_new_range_fixnum(jit, asm, function, opnd!(low), opnd!(high), *flag, &function.frame_state(*state)),
        Insn::ArrayDup { val, state } => objects::gen_array_dup(jit, asm, function, *val, opnd!(val), &function.frame_state(*state)),
        Insn::AdjustBounds { index, length } => gen_adjust_bounds(asm, opnd!(index), opnd!(length)),
        Insn::ArrayAref { array, index, .. } => objects::gen_array_aref(asm, opnd!(array), opnd!(index)),
        Insn::ArrayAset { array, index, val } => {
            no_output!(objects::gen_array_aset(asm, opnd!(array), opnd!(index), opnd!(val)))
        }
        Insn::ArrayPop { array, state } => objects::gen_array_pop(asm, opnd!(array), &function.frame_state(*state)),
        ...
        Insn::StringByteslice { string, beg, len, state } => strings::gen_string_byteslice(asm, opnd!(string), opnd!(beg), opnd!(len), &function.frame_state(*state)),
        ...
        Insn::StringIntern { val, state } => strings::gen_intern(jit, asm, function, opnd!(val), &function.frame_state(*state)),
        ...
        &Insn::Send { cd, block: None, state, reason, .. } => calls::gen_send_without_block(jit, asm, function, cd, &function.frame_state(state), reason),
        &Insn::Send { cd, block: Some(BlockHandler::BlockIseq(blockiseq)), state, reason, .. } => calls::gen_send(jit, asm, function, cd, blockiseq, &function.frame_state(state), reason),
        &Insn::Send { cd, block: Some(BlockHandler::BlockArg), state, reason, .. } => calls::gen_send(jit, asm, function, cd, std::ptr::null(), &function.frame_state(state), reason),
        &Insn::SendForward { cd, blockiseq, state, reason, .. } => calls::gen_send_forward(jit, asm, function, cd, blockiseq, &function.frame_state(state), reason),
        Insn::SendDirect(insn) => {
            let SendDirectData { cd, cme, iseq, recv, args, kw_bits, jit_entry_idx, block, state, .. } = &**insn;
            calls::gen_send_iseq_direct(
                cb, jit, asm,
        ...
        Insn::PushInlineFrame { cme, iseq, recv, num_args, blockiseq, state, .. } => {
            no_output!(calls::gen_push_inline_frame(jit, asm, function, *cme, *iseq, opnd!(recv), *num_args, &function.frame_state(*state), *blockiseq))
        },
        Insn::PopInlineFrame { iseq, argc, state } => {
            no_output!(frame::calls::gen_pop_inline_frame(asm, *iseq, *argc, &function.frame_state(*state)))
        },
        &Insn::InvokeSuper { cd, blockiseq, state, reason, .. } => calls::gen_invokesuper(jit, asm, function, cd, blockiseq, &function.frame_state(state), reason),
        &Insn::InvokeSuperForward { cd, blockiseq, state, reason, .. } => calls::gen_invokesuperforward(jit, asm, function, cd, blockiseq, &function.frame_state(state), reason),
        &Insn::InvokeBlock { cd, state, reason, .. } => calls::gen_invokeblock(jit, asm, function, cd, &function.frame_state(state), reason),
        Insn::InvokeBlockIfunc { cd, block_handler, args, state, .. } => calls::gen_invokeblock_ifunc(jit, asm, function, *cd, opnd!(block_handler), opnds!(args), &function.frame_state(*state)),
        Insn::InvokeProc { recv, args, state, kw_splat } => calls::gen_invokeproc(jit, asm, function, opnd!(recv), opnds!(args), *kw_splat, &function.frame_state(*state)),
        Insn::InvokeBuiltin { bf, leaf, args, state, .. } => calls::gen_invokebuiltin(jit, asm, function, &function.frame_state(*state), unsafe { &**bf }, *leaf, opnds!(args)),
        ...
        &Insn::EntryPoint { jit_entry_idx } => no_output!(frame::gen_entry_point(jit, asm, jit_entry_idx)),
        Insn::Return { val } => no_output!(frame::gen_return(asm, opnd!(val))),
        ...
        &Insn::IsMethodCfunc { val, cd, cfunc, state } => guards::gen_is_method_cfunc(asm, opnd!(val), cd, cfunc, &function.frame_state(state)),
        ...
        Insn::HasType { val, expected } => {
            ...
            guards::gen_has_type(jit, asm, opnd!(val), val_type, *expected)
        }
        &Insn::GuardType { val, guard_type, state, recompile } => {
            let val_type = function.type_of(val);
            guards::gen_guard_type(jit, asm, function, opnd!(val), val_type, guard_type, recompile, &function.frame_state(state))
        }
        &Insn::GuardBitEquals { val, expected, ref reason, state, recompile } => guards::gen_guard_bit_equals(jit, asm, function, opnd!(val), expected, **reason, recompile, &function.frame_state(state)),
        &Insn::GuardAnyBitSet { val, mask, ref reason, state, recompile, .. } => guards::gen_guard_any_bit_set(jit, asm, function, opnd!(val), mask, **reason, recompile, &function.frame_state(state)),
        &Insn::GuardNoBitsSet { val, mask, ref reason, state, .. } => guards::gen_guard_no_bits_set(jit, asm, function, opnd!(val), mask, **reason, &function.frame_state(state)),
        &Insn::GuardLess { left, right, ref reason, state } => guards::gen_guard_less(jit, asm, function, opnd!(left), opnd!(right), **reason, &function.frame_state(state)),
        &Insn::GuardGreaterEq { left, right, state, .. } => guards::gen_guard_greater_eq(jit, asm, function, opnd!(left), opnd!(right), &function.frame_state(state)),
        Insn::PatchPoint { invariant, state } => no_output!(guards::gen_patch_point(jit, asm, function, invariant, &function.frame_state(*state))),
        Insn::CCall { cfunc, recv, args, name, owner, return_type: _, elidable: _ } => calls::gen_ccall(asm, *cfunc, *name, *owner, opnd!(recv), opnds!(args)),
        Insn::CCallWithFrame(insn) => {
            let CCallWithFrameData { cfunc, recv, name, args, cme, state, block, .. } = &**insn;
            calls::gen_ccall_with_frame(jit, asm, function, *cfunc, *name, opnd!(recv), opnds!(args), *cme, *block, &function.frame_state(*state))
        }
        Insn::CCallVariadic(insn) => {
            let CCallVariadicData { cfunc, recv, name, args, cme, state, block, .. } = &**insn;
            calls::gen_ccall_variadic(jit, asm, function, *cfunc, *name, opnd!(recv), opnds!(args), *cme, *block, &function.frame_state(*state))
        }
        Insn::GetIvar { self_val, id, ic, state } => runtime::gen_getivar(asm, opnd!(self_val), *id, *ic, &function.frame_state(*state)),
        Insn::SetGlobal { id, val, state } => no_output!(runtime::gen_setglobal(jit, asm, function, *id, opnd!(val), &function.frame_state(*state))),
        Insn::GetGlobal { id, state } => runtime::gen_getglobal(jit, asm, function, *id, &function.frame_state(*state)),
        &Insn::IsBlockParamModified { flags } => runtime::gen_is_block_param_modified(asm, opnd!(flags)),
        &Insn::GetBlockParam { ep_offset, level, state } => runtime::gen_getblockparam(jit, asm, function, ep_offset, level, &function.frame_state(state)),
        &Insn::SetLocal { val, ep_offset, level, .. } => no_output!(runtime::gen_setlocal(asm, opnd!(val), function.type_of(val), ep_offset, level)),
        Insn::GetConstant { klass, id, allow_nil, state } => runtime::gen_getconstant(jit, asm, function, opnd!(klass), *id, opnd!(allow_nil), &function.frame_state(*state)),
        Insn::GetConstantPath { ic, state } => runtime::gen_get_constant_path(jit, asm, function, *ic, &function.frame_state(*state)),
        Insn::GetClassVar { id, ic, state } => runtime::gen_getclassvar(jit, asm, function, *id, *ic, &function.frame_state(*state)),
        Insn::SetClassVar { id, val, ic, state } => no_output!(runtime::gen_setclassvar(jit, asm, function, *id, opnd!(val), *ic, &function.frame_state(*state))),
        Insn::SetIvar { self_val, id, ic, val, state } => no_output!(runtime::gen_setivar(jit, asm, function, opnd!(self_val), *id, *ic, opnd!(val), &function.frame_state(*state))),
        Insn::FixnumBitCheck { val, index } => scalar::gen_fixnum_bit_check(asm, opnd!(val), *index),
        Insn::SideExit { state, reason, recompile } => no_output!(guards::gen_side_exit(jit, asm, function, reason, *recompile, &function.frame_state(*state))),
        Insn::PutSpecialObject { value_type, state } => runtime::gen_putspecialobject(jit, asm, function, *value_type, &function.frame_state(*state)),
        Insn::AnyToString { val, state } => strings::gen_anytostring(asm, opnd!(val), &function.frame_state(*state)),
        Insn::Defined { op_type, obj, pushval, v, lep_level, state } => runtime::gen_defined(jit, asm, function, *op_type, *obj, *pushval, opnd!(v), *lep_level, &function.frame_state(*state)),
        Insn::CheckMatch { target, pattern, flag, state } => runtime::gen_checkmatch(jit, asm, function, opnd!(target), opnd!(pattern), *flag, &function.frame_state(*state)),
        Insn::GetSpecialSymbol { symbol_type, state } => runtime::gen_getspecial_symbol(asm, *symbol_type, &function.frame_state(*state)),
        Insn::GetSpecialNumber { nth, state } => runtime::gen_getspecial_number(asm, *nth, &function.frame_state(*state)),
        ...
        &Insn::CheckInterrupts { state } => no_output!(runtime::gen_check_interrupts(jit, asm, function, &function.frame_state(state))),
        Insn::BreakPoint => no_output!(asm.breakpoint()),
        Insn::Unreachable => no_output!(asm.abort()),
        &Insn::HashDup { val, state } => { objects::gen_hash_dup(jit, asm, function, val, opnd!(val), &function.frame_state(state)) },
        &Insn::HashAref { hash, key, state } => { objects::gen_hash_aref(jit, asm, function, opnd!(hash), opnd!(key), &function.frame_state(state)) },
        &Insn::HashAset { hash, key, val, state } => { no_output!(objects::gen_hash_aset(jit, asm, function, opnd!(hash), opnd!(key), opnd!(val), &function.frame_state(state))) },
        &Insn::ArrayPush { array, val, state } => { no_output!(objects::gen_array_push(asm, opnd!(array), opnd!(val), &function.frame_state(state))) },
        &Insn::ToNewArray { val, state } => { objects::gen_to_new_array(jit, asm, function, opnd!(val), &function.frame_state(state)) },
        &Insn::ToArray { val, state } => { objects::gen_to_array(jit, asm, function, opnd!(val), &function.frame_state(state)) },
        &Insn::DefinedIvar { self_val, id, pushval, .. } => { runtime::gen_defined_ivar(asm, opnd!(self_val), id, pushval) },
        &Insn::ArrayExtend { left, right, state } => { no_output!(objects::gen_array_extend(jit, asm, function, opnd!(left), opnd!(right), &function.frame_state(state))) },
        Insn::LoadPC => runtime::gen_load_pc(asm),
        Insn::LoadEC => runtime::gen_load_ec(),
        Insn::LoadSP => runtime::gen_load_sp(),
        &Insn::GetEP { level } => runtime::gen_get_ep(asm, level),
        Insn::LoadSelf => runtime::gen_load_self(asm),
        &Insn::LoadField { recv, id, offset, return_type: _, num_bits } => runtime::gen_load_field(asm, opnd!(recv), id, offset, num_bits),
        &Insn::StoreField { recv, id, offset, val, num_bits } => no_output!(runtime::gen_store_field(asm, opnd!(recv), id, offset, opnd!(val), num_bits)),
        &Insn::WriteBarrier { recv, val } => no_output!(runtime::gen_write_barrier(jit, asm, opnd!(recv), opnd!(val), function.type_of(val))),
        &Insn::IsBlockGiven { block_handler } => runtime::gen_is_block_given(asm, opnd!(block_handler)),
        Insn::ArrayInclude { elements, target, state } => objects::gen_array_include(jit, asm, function, opnds!(elements), opnd!(target), &function.frame_state(*state)),
        Insn::ArrayPackBuffer { elements, fmt, buffer, state } => objects::gen_array_pack_buffer(jit, asm, function, opnds!(elements), opnd!(fmt), (*buffer).map(|buffer| opnd!(buffer)), &function.frame_state(*state)),
        &Insn::DupArrayInclude { ary, target, state } => objects::gen_dup_array_include(jit, asm, function, ary, opnd!(target), &function.frame_state(state)),
        Insn::ArrayHash { elements, state } => gen_opt_newarray_hash(jit, asm, function, opnds!(elements), &function.frame_state(*state)),
        &Insn::IsA { val, class } => guards::gen_is_a(jit, asm, opnd!(val), opnd!(class)),
        &Insn::ArrayMax { ref elements, state } => objects::gen_array_max(jit, asm, function, opnds!(elements), &function.frame_state(state)),
        &Insn::ArrayMin { ref elements, state } => objects::gen_array_min(jit, asm, function, opnds!(elements), &function.frame_state(state)),
        &Insn::Throw { throw_state, val, state } => no_output!(frame::gen_throw(jit, asm, function, throw_state, opnd!(val), &function.frame_state(state))),
        &Insn::CondBranch { .. }
...
fn gen_new_array(
    ...
    if unsafe { rb_zjit_array_new_can_fastpath(num, &mut alloc_size, &mut flags) } {
        ...
        return gc_fastpath::gc_fastpath_new_obj(jit, asm, function, state, alloc_size, flags.into(), klass,
            ...
            |asm| {
                frame::gen_prepare_leaf_call_with_gc(asm, state);
                if elements.len() > 0 {
...
fn gen_opt_newarray_hash(
    ...
    frame::gen_prepare_fallback_call(jit, asm, function, state);

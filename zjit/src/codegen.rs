...
/// Capture the types at an exception-handler entry.
fn capture_exception_entry(iseq: IseqPtr, cfp: CfpPtr, insn_idx: u16, stack_size: u8) -> ExceptionEntrySpec {
    let ep = unsafe { get_cfp_ep(cfp) };
    let local_size = unsafe { get_iseq_body_local_table_size(iseq) }.to_usize();
    let mut value_types = Vec::with_capacity(local_size + usize::from(stack_size));
    for local_idx in 0..local_size {
        let ep_offset = local_idx_to_ep_offset(iseq, local_idx) as isize;
        let value = unsafe { ep.offset(-ep_offset).read() };
        value_types.push(exception_entry_value_type(value));
    }
    let base_ptr = unsafe { rb_vm_base_ptr(cfp) };
    for stack_idx in 0..usize::from(stack_size) {
        let value = unsafe { base_ptr.add(stack_idx).read() };
        value_types.push(exception_entry_value_type(value));
    }
    ExceptionEntrySpec { insn_idx, stack_size, value_types }
}

/// Return a useful type that `GuardType` can check without retaining the observed object.
fn exception_entry_value_type(value: VALUE) -> Type {
    if value == Qundef || (!value.special_const_p() && value.builtin_type() == RUBY_T_IMEMO) {
        return types::BasicObject;
    }
    let value_type = Type::from_value(value).unspecialized();
    if value_type.is_immediate()
        || value_type.runtime_exact_ruby_class().is_some()
        || value_type.builtin_type_equivalent().is_some()
    {
        value_type
    } else {
...
c_callable! {
    ...
    fn exception_osr_entry(ec: EcPtr, result: VALUE) -> usize {
        ...
        with_vm_lock(src_loc!(), || {
            let cb = ZJITState::get_code_block();
            let code_ptr = with_time_stat(compile_time_ns, || {
                gen_exception_entry_target_for_current_frame(cb, iseq, ec).map(|(_, target)| target)
            });
            if matches!(&code_ptr, Err(CompileError::OutOfMemory)) {
                unsafe { rb_zjit_compiling_p = false; }
            }
            cb.mark_all_executable();
            code_ptr.map_or(EXCEPTION_OSR_EXIT, |ptr| ptr.raw_addr(cb))
        })
    }
}

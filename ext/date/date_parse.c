...
#ifdef TIGHT_PARSER
# define VALID_DAYS "(?:" DAYS ")" \
                    "|(?:tues|wednes|thurs|thur|" ABBR_DAYS ")\\.?"
# define VALID_MONTHS "(?:" MONTHS ")" \
                      "|(?:sept|" ABBR_MONTHS ")\\.?"
# define DOTLESS_VALID_MONTHS "(?:" MONTHS ")" \
                              "|(?:sept|" ABBR_MONTHS ")"
# define BOS "\\A\\s*"
# define FPA "\\001"
# define FPB "\\002"
...
parse_era2(VALUE str, VALUE hash)
{
    static const char pat_source[] = BEGIN_ERA
      "(c(?:e\\b|\\.e\\.)|b(?:ce\\b|\\.c\\.e\\.)|b(?:c\\b|\\.c\\.))" END_ERA;
    static VALUE pat = Qnil;
...
#ifndef TIGHT_PARSER
      "(?:" BEGIN_ERA
      "(c(?:e|\\.e\\.)|b(?:ce|\\.c\\.e\\.)|a(?:d|\\.d\\.)|b(?:c|\\.c\\.))" END_ERA
      ")?"
      "\\s*"
      "('?-?\\d+(?:(?:st|nd|rd|th)\\b)?)"
#else
...
parse_iso21(VALUE str, VALUE hash)
{
    static const char pat_source[] =
      ...
      BOS
        FPW_COM FPT_COM
      "(\\d{2}|\\d{4})?-?w(\\d{2})(?:-?(\\d))?" TEE_FPT COM_FPW
        EOS
#endif
      ;
    static VALUE pat = Qnil;
...
parse_jis(VALUE str, VALUE hash)
{
    static const char pat_source[] =
      ...
      BOS
        FPW_COM FPT_COM
      "([" JISX0301_ERA_INITIALS "])(\\d+)\\.(\\d+)\\.(\\d+)" TEE_FPT COM_FPW
        EOS
#endif
      ;
    static VALUE pat = Qnil;
...
parse_sla2(VALUE str, VALUE hash)
{
    static const char pat_source[] =
      BOS
        FPW_COM FPT_COM
      "([-']?\\d+)/\\s*(" DOTLESS_VALID_MONTHS ")(?:(?:[-/]|\\s+)\\s*([-']?\\d+))?" COM_FPT COM_FPW
        EOS;
    static VALUE pat = Qnil;
...
parse_sla3(VALUE str, VALUE hash)
{
    static const char pat_source[] =
      BOS
        FPW_COM FPT_COM
      "(" DOTLESS_VALID_MONTHS ")/\\s*([-']?\\d+)(?:(?:[-/]|\\s+)\\s*([-']?\\d+))?" COM_FPT COM_FPW
        EOS;
    static VALUE pat = Qnil;
...
parse_dot2(VALUE str, VALUE hash)
{
    static const char pat_source[] =
      BOS
        FPW_COM FPT_COM
      "([-']?\\d+)\\.\\s*(" DOTLESS_VALID_MONTHS ")(?:(?:[./])\\s*([-']?\\d+))?" COM_FPT COM_FPW
        EOS;
    static VALUE pat = Qnil;
...
parse_dot3(VALUE str, VALUE hash)
{
    static const char pat_source[] =
      BOS
        FPW_COM FPT_COM
      "(" DOTLESS_VALID_MONTHS ")\\.\\s*([-']?\\d+)(?:(?:[./])\\s*([-']?\\d+))?" COM_FPT COM_FPW
        EOS;
    static VALUE pat = Qnil;

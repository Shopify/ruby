...
# if (!defined(__STDC_VERSION__) || __STDC_VERSION__ < 201112 || (defined(__GNUC__) && __GNUC__ < 4 + (__GNUC_MINOR__ < 9) && !defined(__clang__)) || (defined(__clang__) && __clang_major__ < 8))
#  define ALIGN_OF(type) offsetof( \
    struct {char align_c; type align_x; }, align_x)
# else

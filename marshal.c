...
{
    rb_raise(rb_eTypeError, "long too big for this architecture (size " STRINGIZE(SIZEOF_LONG)", given %d)", size);
}
...
{
    VALUE(*hash_new_with_size)
    (st_index_t) = rb_hash_new_with_size;
    VALUE v = Qnil;
    long id;
    st_data_t link;

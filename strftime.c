...
adddecl(static int iso8601wknum(const struct tm *timeptr);) static int weeknumber_v(const struct vtm *vtm, int firstweekday);
...
#define FMT_PADDING(fmt, def_pad) \
 (&"%*" fmt "\0" \
   "%0*" fmt[(padding == '0' || (!padding && (def_pad) == '0')) ? \
       rb_strlen_lit("%*" fmt) + 1 : \
       0])
#define FMT_PRECISION(def_prec) \
 ((flags & BIT_OF(LEFT)) ? (1) : \
     (precision <= 0)    ? (def_prec) : \
                           (precision))

...
impl Context {
    ...
    pub fn stack_push_local(&mut self, local_idx: usize) -> X86Opnd {
        ...
        self.stack_push_mapping((MapToLocal(local_idx as u8), Type::Unknown))
    }
...
pub fn gen_branch(
    ...
    let dst_addr1 = if let Some(ctx1) = ctx1 {
        get_branch_target(target1.unwrap(), ctx1, &branchref, 1, ocb)
    } else {
        None
    };
    ...
    branch.target_ctxs[1] = if let Some(ctx1) = ctx1 {
        *ctx1
    } else {
        Context::default()
    };

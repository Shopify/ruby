...
// The max number of bytes that are generated by emit_conditional_jump
pub const COND_JMP_BYTES: usize = 24;
...
impl Assembler
{
    ...
    pub fn arm64_emit(&mut self, cb: &mut CodeBlock) -> Vec<u32>
    {
        ...
        let mut start_write_pos = cb.get_write_pos();
        ...
        while let Some(insn) = self.insns.get(insn_idx) {
            ...
            match insn {
                ...
                Insn::PosMarker(pos_marker) => {
                    ...
                    if start_write_pos < cb.get_write_pos() {
                        start_write_pos = cb.get_write_pos();
                    }
                }
                ...
                Insn::PadInvalBranch => {
                    while (cb.get_write_pos().saturating_sub(std::cmp::max(start_write_pos, cb.page_start_pos()))) < COND_JMP_BYTES && !cb.has_dropped_bytes() {
                        nop(cb);
                    }
                }
            };

...
#define update_coderange(partial) \
 do { \
  if (coderange != ENC_CODERANGE_BROKEN && scanned < blen && rb_enc_to_index(enc) /* != ENCINDEX_ASCII_8BIT */) { \
   int cr = coderange; \
                    ...
                    if (base != 10) {
                        ...
                        if (sign) {
                            ...
                            for (i = 0; i < RSTRING_LEN(tmp); i++)
                                RSTRING_PTR(tmp)
                            [i] = ruby_digitmap[((unsigned char *)RSTRING_PTR(tmp))[i]];
                            s = RSTRING_PTR(tmp);
                            if (valsign < 0) {
                                sc = '-';
                                width--;
...
#define LITERAL(str) (*sz = rb_strlen_lit(str), str)
        ...
        switch (value) {
#define LITERAL_CASE(x) \
 case Q##x: return LITERAL(#x)
            LITERAL_CASE(nil);
            LITERAL_CASE(true);
            LITERAL_CASE(false);
#undef LITERAL_CASE

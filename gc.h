...
# define SET_MACHINE_STACK_END(p) __asm__ __volatile__("movq\t%%rsp, %0" \
                                                       : "=r"(*(p)))
#elif defined(__i386) && defined(__GNUC__)
# define SET_MACHINE_STACK_END(p) __asm__ __volatile__("movl\t%%esp, %0" \
                                                       : "=r"(*(p)))
...
# define SET_MACHINE_STACK_END(p) __asm__ __volatile__("mr\t%0, %%r1" \
                                                       : "=r"(*(p)))
#elif (defined(__powerpc__) || defined(__powerpc64__)) && defined(__GNUC__) && defined(_AIX)
# define SET_MACHINE_STACK_END(p) __asm__ __volatile__("mr %0,1" \
                                                       : "=r"(*(p)))
#elif defined(__POWERPC__) && defined(__APPLE__) // Darwin ppc and ppc64
# define SET_MACHINE_STACK_END(p) __asm__ volatile("mr %0, r1" \
                                                   : "=r"(*(p)))
#elif defined(__aarch64__) && defined(__GNUC__)
# define SET_MACHINE_STACK_END(p) __asm__ __volatile__("mov\t%0, sp" \
                                                       : "=r"(*(p)))
#else

...
void
dump_memory(uint8_t *base, uint8_t *end)
{
...
bool
find_in_stack(uint32_t target, void *base, void *end)
{
...
void
rb_wasm_mark_mem_range(void *start, void *end)
{
    _rb_wasm_stack_mem[0] = start;
    _rb_wasm_stack_mem[1] = end;
}

#define check_live(target, ctx) \
 do { \
...
void
_check_live(uint32_t target, const char *ctx)
{
...
void
new_frame(uint32_t val, uint32_t depth)
{
    if (depth == 0) {
...
void
take_two_args(uint32_t a, uint32_t b)
{
}

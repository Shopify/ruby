...
newobj_init(VALUE klass, VALUE flags, int wb_protected, rb_objspace_t *objspace, VALUE obj)
{
    ...
    *((VALUE *)&p->as.basic.klass) = klass;
    ...
    RB_VM_LOCK_ENTER_NO_BARRIER();
    {
        ...
        if (RVALUE_REMEMBERED(objspace, obj)) rb_bug("newobj: %s is remembered.", obj_info(obj));
    }
    RB_VM_LOCK_LEAVE_NO_BARRIER();
#endif
...
bool
rb_gc_impl_is_pointer_to_heap(void *objspace_ptr, const void *ptr)
{
    return is_pointer_to_heap((rb_objspace_t *)objspace_ptr, ptr);
}
...
{
    objspace_each_objects((rb_objspace_t *)objspace_ptr, callback, data, TRUE);
}
...
void
rb_gc_copy_finalizer(void *objspace_ptr, VALUE dest, VALUE obj)
{
    rb_objspace_t *objspace = (rb_objspace_t *)objspace_ptr;
    VALUE table;
    st_data_t data;
...
void
rb_gc_impl_free_objects(rb_objspace_t *objspace)
{
...
# define NUM2PTR(x) NUM2ULL(x)
    ...
    object_id = rb_to_int(object_id);
    if (FIXNUM_P(object_id) || rb_big_size(object_id) <= SIZEOF_VOIDP) {
        ptr = NUM2PTR(object_id);
    ...
    if (st_lookup(objspace->id_to_obj_tbl, object_id, &orig) &&
            !rb_gc_impl_garbage_object_p(objspace, orig)) {
        if (!rb_multi_ractor_p() || rb_ractor_shareable_p(orig)) {
            return orig;
        }
        else {
            rb_raise(rb_eRangeError, "%+"PRIsVALUE" is id of the unshareable object on multi-ractor", rb_int2str(object_id, 10));
        }
    }

    if (rb_int_ge(object_id, objspace->next_object_id)) {
        rb_raise(rb_eRangeError, "%+"PRIsVALUE" is not id value", rb_int2str(object_id, 10));
    }
    else {
        rb_raise(rb_eRangeError, "%+"PRIsVALUE" is recycled object", rb_int2str(object_id, 10));
    }
}
...
rb_gc_impl_mark_and_move(void *objspace_ptr, VALUE *ptr)
{
    ...
    if (!is_markable_object(*ptr)) return;
...
void
rb_gc_impl_copy_attributes(VALUE dest, VALUE obj)
{
    if (RVALUE_WB_UNPROTECTED(obj)) {
...
size_t
rb_gc_impl_obj_flags(VALUE obj, ID* flags, size_t max)
{
    size_t n = 0;
    static ID ID_marked;
...
{
    return calloc1(sizeof(ractor_newobj_cache));
}

static void
gc_ractor_newobj_cache_clear(rb_ractor_newobj_cache_t *newobj_cache)
{
...
{
    gc_ractor_newobj_cache_clear(cache);
    free(cache);
}
...
{
    rb_objspace_each_objects(objspace_ptr, gc_set_candidate_object_i, objspace_ptr);
...
{
    return gc_object_moved_p((rb_objspace_t *)objspace_ptr, obj);
}
...
VALUE
rb_gc_impl_location(void *_objspace_ptr, VALUE value)
{
...
root_obj_check_moved_i(const char *category, VALUE obj, void *data)
{
    ...
    if (gc_object_moved_p(objspace, obj)) {
        rb_bug("ROOT %s points to MOVED: %p -> %s", category, (void *)obj, obj_info(rb_gc_impl_location(obj)));
    }
}
...
reachable_object_check_moved_i(VALUE ref, void *data)
{
    ...
    if (gc_object_moved_p(rb_gc_get_objspace(), ref)) {
        rb_bug("Object %s points to MOVED: %p -> %s", obj_info(parent), (void *)ref, obj_info(rb_gc_impl_location(ref)));
    }
}
...
rb_raw_obj_info_common(char *const buff, const size_t buff_size, const VALUE obj)
{
    ...
    if (SPECIAL_CONST_P(obj)) {
        ...
        if (rb_gc_impl_is_pointer_to_heap(gc_get_objspace(), (void *)obj)) {
    ...
    if (LIKELY(pos < buff_size) && !SPECIAL_CONST_P(obj)) {
        ...
        switch (type) {
          ...
          case T_ARRAY:
            if (ARY_SHARED_P(obj)) {
                APPEND_S("shared -> ");
                gc_raw_obj_info(BUFF_ARGS, ARY_SHARED_ROOT(obj));
            }
          ...
          case T_MOVED: {
            APPEND_F("-> %p", (void*)rb_gc_impl_location(gc_get_objspace(), obj));
            break;
          }
          case T_HASH: {
            APPEND_F("[%c] %"PRIdSIZE,
                    ...
                    if (me->def) {
                        switch (me->def->type) {
                          case VM_METHOD_TYPE_ISEQ:
                            APPEND_S(" (iseq:");
                            gc_raw_obj_info(BUFF_ARGS, (VALUE)me->def->body.iseq.iseqptr);
                            APPEND_S(")");
                            break;
                          default:
                            break;
...
static const char *
gc_raw_obj_info(char *const buff, const size_t buff_size, VALUE obj)
{
...
obj_info(VALUE obj)
{
    ...
    return gc_raw_obj_info(buff, OBJ_INFO_BUFFERS_SIZE, obj);
}
...
const char *
rb_gc_impl_obj_info(void *objspace_ptr, VALUE obj)
{
    return obj_info(obj);
}

const char *
rb_gc_impl_full_obj_info(void *objspace_ptr, VALUE obj, char *buffer, size_t buffer_size)
{
    return gc_raw_obj_info(buffer, buffer_size, obj);
}

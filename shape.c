...
static rb_shape_t*
get_next_shape_internal(rb_shape_t * shape, ID id, enum shape_type shape_type, bool * leaf_created)
{
    rb_shape_t *res = NULL;
    RB_VM_LOCK_ENTER();
    {
...
rb_shape_get_next(rb_shape_t* shape, VALUE obj, ID id)
{
    ...
    bool leaf_created;
    rb_shape_t * new_shape = get_next_shape_internal(shape, id, SHAPE_IVAR, &leaf_created);
    ...
    if (BUILTIN_TYPE(obj) == T_OBJECT) {
        ...
        if (leaf_created) {
            RCLASS_EXT(klass)->shape_leaf_count++;
        }
    }

...
shape_alloc(void)
{
    rb_shape_tree_t *tree = GET_SHAPE_TREE();
    shape_id_t shape_id = tree->next_shape_id;
    tree->next_shape_id++;
    ...
    return &tree->shape_list[shape_id];
}
...
#else
    rb_shape_tree_ptr->shape_list = xcalloc(SHAPE_BUFFER_SIZE, sizeof(rb_shape_t));
#endif

...
gc_set_initial_pages(rb_objspace_t *objspace, int global_heap_init_slots)
{
    ...
    for (int i = 0; i < SIZE_POOL_COUNT; i++) {
        rb_size_pool_t *size_pool = &size_pools[i];
        char env_key[40];
        sprintf(env_key, "RUBY_GC_HEAP_INIT_SIZE_%d_SLOTS", size_pool->slot_size);
        size_t pool_init_slots = global_heap_init_slots ? gc_params.heap_init_slots : 0;
        get_envparam_size(env_key, &pool_init_slots, 0);
